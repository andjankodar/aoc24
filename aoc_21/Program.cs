...
foreach (var code in codes)
{
    ...
    var rb2NeedsToPress = RemoteToDoorpad(code);    
    var rb1NeedsToPress = RemoteToRemote(rb2NeedsToPress);
    var iNeedToPress = RemoteToRemote(rb1NeedsToPress);
    
    Console.WriteLine(iNeedToPress);
    Console.WriteLine(rb1NeedsToPress);
    Console.WriteLine(rb2NeedsToPress);
...
string RemoteToRemote(string input)
{
    ...
    for (var j = 0; j < input.Length; j++)
    {
        ...
        Console.WriteLine($"{input}: {sequence + "A"}");
        output.Append(sequence + "A");
