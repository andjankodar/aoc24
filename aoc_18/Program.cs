...
Console.WriteLine($"Reached end in {fewestSteps} steps");

for (int b = 1025; b < memSpace.Count;)
{ 
    foreach (var step in bestPath)
    {
        if (memSpace.Take(b).Contains(step))
        {
            Console.WriteLine($"Previous best path blocked by {memSpace[b-1]}");
            grid = GetMaze(71, b);
            fewestSteps = null;
            stepsToPos = new Dictionary<(int, int), long>();
            fewestSteps = null;
    ...
    if (fewestSteps == null)
    {        
        Console.WriteLine($"Could not reach end");
        break;
    }
...
void PrintPath(List<(int row, int col)> visited )
{
    for (int i = 0; i < grid.Length; i++)
    {
        for (int j = 0; j < grid[i].Length; j++)
