...
var longestNetwork = new HashSet<string>();
long total = 0;
...
var result = longestNetwork.ToArray();
var sorted = longestNetwork.ToList();
sorted.Sort();
Console.WriteLine($"Longest network {string.Join(",", sorted)}. {sorted.Count} {Stopwatch.GetElapsedTime(start)}");
...
void BuildNetwork(ImmutableHashSet<string> network)
{
    ...
    if (longestNetwork.Count() < network.Count())
    {
        longestNetwork = network.ToHashSet();
    }
    ...
    foreach(var comp in network)
    {
        foreach (var computer in connectionDictionary[comp])
        {
            ...
            if (connectionDictionary[computer].Count() <= longestNetwork.Count() - 1)
            {
                continue;
            }
            ...
            {
                // This computer is connected to all nodes in network. Add and continue building
